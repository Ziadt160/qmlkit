...
print(f"circuits run: {kernel.n_evaluations}  (one per row: m = {len(X_train)})")
print(f"on a device that would be: {kernel.circuits_on_hardware}  (m(m-1)/2)")
...
circuits run: 28  (one per row: m = 28)
on a device that would be: 378  (m(m-1)/2)
target alignment: +0.3360
```

Those two numbers are both real and they are not the same number.

The inversion test reads `P(0…0)` of `U(x′)† U(x)|0⟩`, which *is* `|⟨ψ(x′)|ψ(x)⟩|²`.
A device has no choice but to build that composed circuit once per pair, because it
cannot hand you a state. A simulator can hand you one — so `QuantumKernel` evaluates
each row **once** and forms every overlap with a single matrix product. That is the
difference between linear and quadratic in the size of your dataset, and it is why a
1000-point Gram matrix is a thousand circuits here rather than half a million.

`n_evaluations` is what actually ran. **`circuits_on_hardware` is the number to budget
a real run from** — the shortcut above is simulator-only, and taking the first number
to a device would under-count it by a factor of `(m−1)/2`.

The row-keyed cache is the other half of this: predicting against the training set
re-encodes only the test points, because the training states are already in hand.
